...
export default class Composer extends Disposable {
  ...
  async getDiCy (filePath, shouldRebuild = false, fastLoad = false) {
    ...
    .on('command', event => {
      latex.log.info(`[${event.rule}] Executing \`${event.command}\``, filePath)
    })
    .on('fileDeleted', event => {
      if (!event.virtual) {
        latex.log.info(`Deleting \`${event.file}\``, filePath)
      }
    })
  ...
  async runDiCy (commands, { shouldRebuild, fastLoad, openResults, busyText, clearLog } = { shouldRebuild: false, fastLoad: false, openResults: true, clearLog: false }) {
    ...
    if (!fastLoad) latex.status.setBusy(busyText)
  ...
  async build (shouldRebuild = false, enableLogging = true) {
    ...
    const [, relativeFilePath] = atom.project.relativizePath(filePath)
    const busyText = `LaTeX: ${shouldRebuild ? 'Rebuilding' : 'Building'} ${relativeFilePath}`
  ...
  async clean () {
    ...
    const [, relativeFilePath] = atom.project.relativizePath(filePath)
    const busyText = `LaTeX: Cleaning ${relativeFilePath}`

    if (this.shouldUseDiCy()) {
      return this.runDiCy(['load', 'clean', 'save'], { busyText, openResults: false, clearLog: true })
    }
