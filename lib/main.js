...
export default {
  ...
  consumeBusySignal (service) {
    this.bootstrap()
    latex.status.attachBusySignal(service)
    return new Disposable(() => {
      if (global.latex) {
        global.latex.status.detachBusySignal()
      }
    })
  },
  ...
  consumeLinterIndie (register) {
    this.bootstrap()
    const LinterDispatcher = require('./linter-dispatcher')
    const dispatcher = new LinterDispatcher(register)
    this.disposables.add(dispatcher)
    return dispatcher
  },
